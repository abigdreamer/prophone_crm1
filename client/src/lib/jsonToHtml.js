// Converts the template JSON structure into email-safe HTML (table-based layout)
...
function bgAttr(bg) {
  ...
  if (bg.type === "gradient") {
    ...
    return { style: `background:linear-gradient(${g.angle ?? 135}deg,${from},${g.to || "#818cf8"});`, bgcolor: from };
  }
...
export function jsonToHtml(structure) {
  const {
