...
export default function DashboardPage({
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        ...
        // 2. Fetch Dashboard Stats (Uses singleton pool/clientId inside the service)
        const summaryData = await db.getDashboardSummary();
        setSummary({
  ...
  // Lead Breakdown Logic
  const leads = contacts.filter((c) => LEAD_STAGES.includes(c.lifecycleStage));
  const stageMap = {};
  contacts.forEach((c) => {
    stageMap[c.lifecycleStage] = (stageMap[c.lifecycleStage] || 0) + 1;
  });
