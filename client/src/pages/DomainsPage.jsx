...
function ClientChip({ clientId }) {
  ...
  const c = clientId ? clients.find((cl) => cl.id === clientId) || null : null;
  if (!clientId || !c)
    return (
      <span
        style={{
...
function DetailPanel({ domain, onClose, onDeleted, onUpdated, onCanceled, onRestored }) {
  ...
  async function handleSaveFromEmail() {
    setSavingFrom(true);
    try {
      const updated = await db.updateDomain(domain.id, {
  ...
  return (
    ...
    >
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '16px 20px' }}>
        ...
        <DnsBlock
          label="SPF"
          recordJson={domain.spfRecord}
          status={domain.status}
        />
        <DnsBlock
          label="DKIM"
          recordJson={domain.dkimRecord}
          status={domain.status}
        />
        <DnsBlock
          label="DMARC"
          recordJson={domain.dmarcRecord}
          status={domain.status}
        />
        ...
        {domain.status !== 'verified' && (
          ...
          >
            ...
            <span style={{ fontSize: 11, color: T.amber, lineHeight: 1.5 }}>
              Add the records above to your DNS provider. Changes can take up to
              24 hours to propagate. We re-verify automatically every 30
              seconds.
            </span>
          </div>
        )}
          ...
          <>
            ...
            <div style={{ marginBottom: 6 }}>
              <div style={{ fontSize: 11, color: T.muted, marginBottom: 6 }}>
                Default From Email — used as the sender when no custom address
                is set.
              </div>
                ...
                {fromEmailDirty && (
                  <button
                    ...
                    style={{
                      ...
                      fontSize: 12,
                      fontWeight: 600,
                      cursor: savingFrom ? 'not-allowed' : 'pointer',
                      fontFamily: 'inherit',
  ...
  if (newDomain) {
    return (
      <Modal title="DNS Records" onClose={onClose} width={580}>
        ...
        >
          <Check size={14} />
          <strong>{newDomain.domainName}</strong> added. Add these DNS records
          to verify ownership.
        </div>
        {[
        ...
        >
          <AlertTriangle size={13} /> Add these records to your DNS provider
    ...
    <Modal noHeader onClose={onClose} width={500}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
          ...
          <div>
            ...
            <div style={{ fontSize: 12, color: T.muted }}>
              DNS records will be generated next
            </div>
          </div>
        </div>
        <button
      ...
      <div style={{ marginBottom: 18 }}>
        ...
        <div style={{ fontSize: 11, color: T.muted, marginTop: 6 }}>
          Use a subdomain (e.g.{' '}
          <code style={{ fontFamily: 'monospace', color: T.dim }}>mail.</code>)
          to keep your root domain isolated.
        </div>
      </div>
...
export default function DomainsPage() {
  ...
  function handleAdded(domain) {
    setDomains((prev) =>
      prev.some((d) => d.id === domain.id) ? prev : [domain, ...prev],
    );
  }
      ...
      >
        ...
        >
          <div>
            ...
            <div style={{ fontSize: 13, color: T.muted }}>
              Verify domains so your emails pass SPF, DKIM and DMARC checks.
            </div>
          </div>
          ...
          >
            ...
            <div style={{ fontSize: 12, color: T.muted, marginBottom: 18 }}>
              {search
                ...
                : 'Add and verify a domain to start sending emails.'}
            </div>
            {!search && (
              <button
                onClick={() => setShowModal(true)}
        ...
        {!loading && (
          ...
          >
            ...
            />
            Auto-refreshes every 30s · last sync{' '}
